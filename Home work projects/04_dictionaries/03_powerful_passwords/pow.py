...
def login(email, stored_logins, password_to_check):
    """
    Returns True if the hash of the password matches the one in stored_logins
...
def hash_password(password):
    """
    Takes a password and returns its SHA256 hashed value.
    """
    return sha256(password.encode()).hexdigest()


def main():
    # Dictionary of emails and their corresponding hashed passwords
    stored_logins = {
